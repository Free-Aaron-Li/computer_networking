...
#indent 最便宜且最常用的导引型传输媒介，在一百来年的历史中，一直用于电话网。通过两根线（构成了一条通信链路）绞合在一起，以减少邻近类似的双绞线的*电气干扰*，并在这些双绞线外覆盖上保护性防护层。无屏蔽双绞线常用在建筑内的计算机网络中，即局域网中。目前局域网中双绞线的传播速率为10M
bps \~ 10G
...
#indent 虽然现如今光纤被广泛运用，但是在高速LAN环境下，双绞线仍然是主导性解决方案。
...
#indent 同轴电缆由两根铜导体组成（与双绞线类似，但是导体是同心而非并行）。同轴电缆在电缆电视系统中应用相当普遍。

在电缆电视和电缆因特网接入中，发送设备将数字信号调制到某个特定的频段，产生的模拟信号从发送设备传送到一个或多个接收方。同轴电缆能被用作导引型*共享媒介*（shared medium）。
